...
Dependency trigger: rows 02 and 03 require FastAPI, Pydantic, and pytest. POST rate limiting is implemented as local in-memory S02 API code under the row 03 write roots, not through an unverified package dependency. If any required package dependency is missing and passing the slice would require package-manifest edits, execution blocks because package-file writes are outside S02 authority.
...
Implement the POST mood rate limiter as local in-memory S02 API code under `src/api/` and cover it with deterministic tests. Do not add package-manifest edits, hidden external services, broad write roots, or GET endpoint rate limits.
...
Dependency availability is unverified. If FastAPI, Pydantic, or pytest are unavailable and passing the slice requires package-file edits, S02 blocks pending new authority. The POST mood rate limiter must not rely on an unverified package dependency.
